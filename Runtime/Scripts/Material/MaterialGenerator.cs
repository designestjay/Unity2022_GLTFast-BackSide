...
namespace GLTFast.Materials
{
    ...
    public abstract class MaterialGenerator : IMaterialGenerator
    {
        ...
        public static readonly int ClearCoatMaskProperty = Shader.PropertyToID("clearcoat_mask");
        public static readonly int ClearCoatRoughnessProperty = Shader.PropertyToID("clearcoat_roughness");
            ...
            )
        {
            ...
            if (textureInfo.extensions?.KHR_texture_transform != null)
            {
                ...
                if (tt.rotation != 0)
                {
                    ...
                    // Note: this is a fix to handle rotated textures with negative -y values, specifically to handle a case in v536
                    // If this is fixed in the original 3D model this line can be restored.
                    // There are currently no correctly rotated materials, so this might cause trouble if other models are added.
