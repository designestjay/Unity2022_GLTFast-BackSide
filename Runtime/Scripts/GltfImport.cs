    ...
    public class GltfImport : IGltfReadable, IGltfBuffers, IDisposable
    {
        ...
        async Task InstantiateSceneInternal(Root gltf, IInstantiator instantiator, int sceneId)
        {
            ...
            void PopulateHierarchy(uint nodeIndex, uint? parentIndex)
            {
                ...
                if (!node.extras.AllNullEX90())
                {
                    GameObjectInstantiator goi = instantiator as GameObjectInstantiator;
                    var extraData = goi.GetGameobject(nodeIndex).AddComponent<ExtraData>();
                    extraData.Populate(node.extras);
                }

                if (!node.extras.AllNullEX30())
                {
                    GameObjectInstantiator goi = instantiator as GameObjectInstantiator;
                    var extraData = goi.GetGameobject(nodeIndex).AddComponent<ExtraDataEX30>();
                    extraData.Populate(node.extras);
                }
